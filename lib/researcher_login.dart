...
// --- Design System Constants (Self-Contained for Researcher Theme) ---
// We define a new color palette here to give this page a unique, professional feel
// while keeping the layout and structure consistent.
...
/// Consistent spacing values for layout (identical across all pages for consistency).
class _AppSpacings {
...
class _ResearcherLoginPageState extends State<ResearcherLoginPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _login() async {
    ...
    try {
      // Using the specific login logic for researchers
      ...
      if (mounted) {
        final details = await _blockchainService.getResearcherDetails(_usernameController.text.trim());
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => HomePage(userProfile: userProfile, userRole: UserRole.researcher),
          ),
        );
      }
    } catch (e) {
    ...
    return Container(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.science_outlined, color: _AppColors.primary, size: 50), // Thematic icon
            const SizedBox(height: _AppSpacings.m),
            const Text(
              "Researcher Portal", // Thematic title
              style: TextStyle(
            ...
            const Text(
              "Sign in to the research hub", // Thematic subtitle
...
/// The custom painter for the blue header wave.
class HeaderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Using the new researcher color theme for the gradient
